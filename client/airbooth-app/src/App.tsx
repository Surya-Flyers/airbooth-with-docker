...
function App() {
  ...
  const joinCall = () => {
    console.log("join-call-E - callId : ", callDetails.current._id);
    socketIoRef.current.emit(
      "join-call-E",
      { callId: callDetails.current._id, userId: userName },
      (data) => {
      ...
      try {
        ...
        await socketIoRef.current.emit(
          ...
          ({ id, producersExist }) => {
            ...
            callback({ id });
            console.log(
              "transport-produce-E producersExist : ",
              producersExist
            );
  ...
  async function initiateCall() {
    ...
    socket.emit(
      ...
      ({ data }: any) => {
        ...
        console.log("T1 callDetails.current : ", callDetails.current);
      }
    );

    socket.on("inspector-found", ({ producerId, inspectorId }: any) => {
      console.log("inspector-found producerId : ", producerId);
      console.log("inspector-found inspectorId : ", inspectorId);
      callDetails.current = {
        ...callDetails.current,
  ...
  function listenToItems() {
    socketIoRef.current.emit("get-items", {
      userId: userName,
      callId: callDetails.current._id,
      userType: "TOURIST",
    });
